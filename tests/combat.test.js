...
test('buffMultipliers — 버프 타워 반경 내면 공격력↑·공속 오버라이드', () => {
  ...
  assert.equal(m.dmgMult, 1.3);
  assert.equal(m.atkSpeed, 1.8);
});
...
test('effectiveSpeed — 슬로우 중 ×SLOW_FACTOR, 스턴 중 0', () => {
  const Combat = require('../src/combat.js');
  const e = { baseSpeed: 1, slowUntil: 10, stunUntil: 0 };
  assert.equal(Combat.effectiveSpeed(e, 5), 0.5);  // 슬로우 중(-50%)
  assert.equal(Combat.effectiveSpeed(e, 11), 1.0); // 만료
  const s = { baseSpeed: 1, slowUntil: 0, stunUntil: 8 };
  assert.equal(Combat.effectiveSpeed(s, 5), 0);    // 스턴 중 정지
  assert.equal(Combat.effectiveSpeed(s, 9), 1.0);  // 만료
});

test('resolveHit — 스턴은 stunUntil 부여', () => {
  const Combat = require('../src/combat.js');
  const e = { hp: 100, slowUntil: 0, stunUntil: 0 };
  Combat.resolveHit({ atkType: 'stun', damage: 30 }, e, 4);
  assert.equal(e.hp, 70);
  assert.equal(e.stunUntil, 5); // now(4) + STUN_DURATION(1)
});
